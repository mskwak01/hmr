...
class HMRTrainer(object):
    ...
    def _create_2d_data_loader(self, data_2d_set):
        data_set = []
        # data_2d_set = data_2d_set.remove('ai-ch')
        # print(f'type of data_2d_set is {type(data_2d_set)}')
        # print(f'data_2d_set is {data_2d_set}')
        data_2d_set.remove('ai-ch')
        # print(f'data_2d_set is {data_2d_set}')
    ...
    #         shuffle = True,
    #         drop_last = True,
    #         # pin_memory = True,is:issue is:closed)
    #         pin_memory = True)
    ...
    def train(self):
        ...
        for iter_index in range(config.args.iter_count):
            ...
            sample_2d_count = image_from_2d.shape[0]

            # images = torch.cat((image_from_2d, image_from_3d), dim = 0).cuda()
            images = image_from_2d.clone().cuda()
            ...
            loss_kp_2d = self._calc_loss(generator_outputs, data_2d)
            ...
            if iter_index % 500 == 0:
                iter_msg['title'] = '{}_{}_'.format(iter_msg['iter'], iter_msg['2d_loss'])
                save_model(iter_msg)
    ...
    def _calc_loss(self, generator_outputs, data_2d):
        ...
        real_2d = data_2d['kp_2d'].clone().float().cuda()
        predict_j2d = predict_j2d
