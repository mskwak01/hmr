...
class HMRNetBase(nn.Module):
    def __init__(self):
        ...
        self.smpl = SMPL('./SMPL_weight/basicModel_f_lbs_10_207_0_v1.1.0.pkl').to(self.device)
    ...
    def _calc_detail_info(self, theta):     # theta = big theta that includes cam, pose, shape
        # print(f'len theta is {theta.size()}')
        cam = theta[:, 0:3].contiguous()
        pose = theta[:, 3:75].contiguous()      # 72 (24, 3)
        shape = theta[:, 75:].contiguous()
        
        _, verts, j3d, Rs = self.smpl(betas = shape, body_pose = pose, get_skin = True)
